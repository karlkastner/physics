...
%% output
%% fbs :
function [fbs,chi] = backscatter_form_function(d_mm,f_Hz,mode,varargin)
	...
	chi = normalized_particle_radius(d_mm,f_Hz,varargin{:});
	...
	case {'medwin-clay-1997'}
		...
		e   = 39;
		% f ~ 1.17
		fbs_ = 2*( (e-1)./(3*e) + (rho_s-rho_w)./(2*rho_s+rho_w) );
		fbs  = fbs_*chi.^2;
	case {'hay-sheng-1988'}
 	...
 	case {'thorne-manley-1993'}
		...
		ks  = 1/38;
		gk  = (ks-kw)/kw;
		gr  = 3*(rho_s-rho_w)/(2*rho_s+rho_w);
		kf  = 2/3*(gk - gr);
		else
			kf = 1.1; % from thorne 1993
		end
	...
	case {'moate-thorne-2012'}
		...
		fbs =      (1-d1*exp(-((chi-d2)/d3).^2)) ...
			...
			fbs = sqrt(rho_s).*(1-0.25*exp(-((chi-1.5)/0.35).^2)) ...
			         .*(1+0.60*exp(-((chi-2.9)/1.15).^2)) ...
				 .*chi.^2./(42 + 28*chi.^2);
		end
	case {'thorne-2014'}
			% TODO, there is a mistake in the reference 
			% apparently Thorne changed the notation, K != ks
		fbs = sqrt(rho_s) ...
