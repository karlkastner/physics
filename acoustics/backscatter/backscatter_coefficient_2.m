...
function [ks2, fbs, chi, D1] = backscatter_coefficient_2(d_mm,f_Hz,varargin)
	d_m = 1e-3*d_mm;
	if (~issym(d_mm))
	...
	[fbs, chi] = backscatter_form_function(d_mm,f_Hz,varargin{:});
	% thorne 2008, 1
	% note that thorne-2014 has 3/(16 pi) in the system constant
	% note that for a distribution the integral is :
	% ks2 ~ <d^2 f^2>/<d^3>
